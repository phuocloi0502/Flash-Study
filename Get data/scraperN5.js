...
for (let chapter = 1; chapter <= 10; chapter++) {
  for (let section = 1; section <= 5; section++) {
    const url = `https://jtest.net/tu-vung-n5/chapter-${chapter}/section-${section}`;
    links.push(url);
  }
}

// Biến đếm lesson_id bắt đầu từ 61
let lessonIdCounter = 97;
...
async function getAllVocabularyData() {
  ...
  fs.writeFileSync(
    "vocabulary_data_n5.sql",
    allSqlCommands.join("\n"),
    "utf-8"
  );
  console.log("Dữ liệu đã được lưu vào vocabulary_data_n5.sql");
}
